...
var connection = $.connection.webRTCHub;
...
connection.client.updateRoom = function (data) {
    var obj = JSON.parse(data);
    $(roomTable).DataTable().clear().rows.add(obj).draw();
};

connection.client.created = function (roomId) {
...
connection.client.joined = function (roomId) {
    console.log('This peer has joined room', roomId);
    myRoomId = roomId;
    isInitiator = false;
};

connection.client.error = function (message) {
    alert(message);
};

connection.client.ready = function () {
...
connection.client.bye = function () {
...
$(window).on('beforeunload', function () {
    if (hasRoomJoined) {
        ...
        connection.server.leaveRoom(myRoomId);
    }
});

// Start connection and invoke methods after SignalR connection is established
$.connection.hub.start().done(function () {
    // Get room list
    connection.server.getRoomInfo().fail(function (err) {
...
function sendMessage(message) {
    ...
    connection.server.sendMessage(myRoomId, message).fail(function (err) {
        console.error(err.toString());
    });
}
...
$(createRoomBtn).click(function () {
    ...
    connection.server.createRoom(name).fail(function (err) {
        console.error(err.toString());
    });
});
...
$('#roomTable tbody').on('click', 'button', function () {
    if (hasRoomJoined) {
        ...
        connection.server.join(data.RoomId).fail(function (err) {
            console.error(err.toString());
        });
    }
});
